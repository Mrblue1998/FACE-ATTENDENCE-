...
class FacesDatabase:
        ...
        while 1:
            ...
            if k == 13:  # Enter
                if len(name) > 0:
                    ...
                    cv2.putText(cc, "Name was not inserted. Please try again.", (30, 200), font, fontScale, fontColor,
                                lineType)
                    cv2.imshow(winname, cc)
                    k = cv2.waitKey(0)
                    if k == 27:
    ...
    def create_new_label(self, path, id):
        while osp.exists(osp.join(path, "face{}.jpeg".format(id))):
            id += 1
    ...
    def dump_faces(self, image, desc, name):
        ...
        if match < 0:
            filename = "{}-0.jpeg".format(label)
            match = len(self.database) - 1
        else:
            filename = "{}-{}.jpeg".format(label, len(self.database[match].descriptors) - 1)
